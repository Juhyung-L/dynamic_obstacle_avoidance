...
{ 
    ...
    void GlobalTrajectoryAlignCritic::prepare(const nav_2d_msgs::msg::Path2D& traj)
    {
        makeAlignmentMap(traj);
    }

    double GlobalTrajectoryAlignCritic::scoreTrajectory(const nav_2d_msgs::msg::Path2D& traj)
    {
        score_normalizer_.setMaxScore(
            static_cast<double>(UCHAR_MAX) * static_cast<double>(traj.poses.size()));

        double score = 0.0;
        for (auto& pose : traj.poses)
        {
            unsigned int x, y;
